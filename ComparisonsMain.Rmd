...
laligned = read.csv(file.path(dir.e, paste(family, "_core_", core, "_out_m.laligned.csv", sep = "")))$x
  index <- c()
...
m.Pne = read.csv(file.path(dir.e, paste(family, "_core_", core, "_out_m.Pn.csv", sep = "")))[index, 1:nmodes4]
Pne.mean = colMeans(m.Pne)
m.Qne = t(apply(m.Pne, 1, cumsum))
Qne.mean = colMeans(m.Qne)
m.evaluese = read.csv(file.path(dir.e, paste(family, "_core_", core, "_out_m.evalues.csv", sep = "")))[index, 1:nmodes4]
evaluese.mean = colMeans(m.evaluese)
ievaluese.mean = 1/evaluese.mean

MSD = read.csv(file.path(dir.e, paste(family, "_core_", core, "_out_MSD.csv", sep = "")))$x
id = read.csv(file.path(dir.e, paste(family, "_core_", core, "_out_m.ID.csv", sep = "")))$x
id.index = id[index]

# Read theoretical output.
dir.t <- "OUT/Theoretical"
m.Pnt = read.csv(file.path(dir.t, paste(family, "_core_", core, "_out_m.Pn.csv", sep = "")))[, 1:nmodes4]
m.Qnt = t(apply(m.Pnt, 1, cumsum))
evalues = read.csv(file.path(dir.t, paste(family, "_core_", core, "_out_evalues.csv", sep = "")))$x[1:nmodes4]
ievalues = 1/evalues
...
### Conjunto teórico
`r nmut` mutantes de `r p.ref` de `r round(mean(id.index))` % de identidad secuencial con ANM *R0* = 12 y *Fmax* = 2. Se seleccionó esta proteína ya que es la que presenta menor *MSD* con respecto a las coordenadas xyz promedio de todas las proteínas del conjunto experimental. El porcentaje de mutaciones se seleccionó de acuerdo al promedio de identidad secuencial de proteínas del conjunto experimental. 

### Conjunto experimental 
`r nexp` proteínas de la familia de `r family`. Las proteínas tienen un promedio de identidad secuencial de `r round(mean(id.index))`%. Todas las proteínas del sub-conjunto elegidas alinean en todos los sitios de `r p.ref`, o en como máximo 1 sitio menos.
...
plot(Pnt.group.mean, ylab = "Pn promedio", xlab = "nmodo", type = "l", ylim = c(0, 0.07), main = "Pn promedio vs. nmodo")
lines(Pnt.group.quantile5, col = "green")
lines(Pnt.group.quantile95, col = "green")
points(Pne.mean, col = "red")
plot(Qnt.group.mean, ylab = "Qn promedio", xlab = "nmodo", type = "l", ylim = c(0, 0.7), main = "Qn promedio vs. nmodo")
