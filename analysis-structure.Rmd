...
site = site.info$site
dri2.medium = dri2.medium[1:length(protein.exp), site]
...
p.accept.medium.sel = sim.data.medium.sel$p.accept

# calculate mean.p.accept of simulated data
mean.p.accept.no.sel = c()
mean.p.accept.medium.sel = c()
...
for (i in (1:n.sites)) {
  ...
  p.accept.medium.sel.i = p.accept.medium.sel[which(mut.site.medium.sel == i)]
  mean.p.accept.medium.sel.i = mean(p.accept.medium.sel.i)
  mean.p.accept.medium.sel[i] = mean.p.accept.medium.sel.i
}
...
plot(x = log(mean.p.accept.medium.sel), y = CN)
plot(x = log(mean.p.accept.medium.sel), y = score)
plot(x = score, y = CN)

## calculate corelations
r.mean.p.accept.medium.sel.CN = cor(log(mean.p.accept.medium.sel), CN)
r.mean.p.accept.medium.sel.score = cor(log(mean.p.accept.medium.sel), score)
r.score.CN = cor(score, CN)

cor.df = data.frame("comparison" = c("log(p.accept.medium.sel) vs CN", "log(p.accept.medium.sel) vs score", "score vs CN"),
                    "R" = c(r.mean.p.accept.medium.sel.CN , r.mean.p.accept.medium.sel.score, r.score.CN))
...
plot(x = d.medium$mean.z.RSD, y = CN, xlab = "theoretical with medium selection <zRSDi>")
...
cor.df = data.frame("comparison" = c("exp.mean.z.RSD vs CN", "mut.mean.z.RSD vs CN", "medium.mean.z.RSD vs CN"),
                    "R" = c(r.exp.mean.z.RSD.CN , r.mut.mean.z.RSD.CN, r.medium.mean.z.RSD.CN))

knitr::kable(cor.df, digits = 2, caption = "Correlation structure - sequence")
```
