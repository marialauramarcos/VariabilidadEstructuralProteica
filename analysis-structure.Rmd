...
plot(x = d.exp$mean.z.RSD, y = (1/CN), xlab = "<zRSDi> experimental")
plot(x = d.mut$mean.z.RSD, y = (1/CN), xlab = "<zRSDi> no-selection")
plot(x = d.medium$mean.z.RSD, y = (1/CN), xlab = "<zRSDi> selection")

## calculate correlations
r.exp.mean.z.RSD.CN = cor(d.exp$mean.z.RSD, (1/CN))
r.mut.mean.z.RSD.CN = cor(d.mut$mean.z.RSD, (1/CN))
r.medium.mean.z.RSD.CN = cor(d.medium$mean.z.RSD, (1/CN))

cor.df = data.frame("comparison" = c("<z.RSD> experimental vs 1/CN", "<z.RSD> no-selection vs log(CN)", "<z.RSD> selection vs 1/CN"),
                    "R" = c(r.exp.mean.z.RSD.CN , r.mut.mean.z.RSD.CN, r.medium.mean.z.RSD.CN))

knitr::kable(cor.df, digits = 2, caption = "Pearson Correlation structure (<z.RSD>) - sequence (1/CN)")

## calculate correlations
r.exp.mean.z.RSD.CN = cor(d.exp$mean.z.RSD, (1/CN), method = "spearman")
r.mut.mean.z.RSD.CN = cor(d.mut$mean.z.RSD, (1/CN), method = "spearman")
r.medium.mean.z.RSD.CN = cor(d.medium$mean.z.RSD, (1/CN), method = "spearman")

cor.df = data.frame("comparison" = c("<z.RSD> experimental vs 1/CN", "<z.RSD> no-selection vs 1/CN", "<z.RSD> selection vs 1/CN"),
                    "R" = c(r.exp.mean.z.RSD.CN , r.mut.mean.z.RSD.CN, r.medium.mean.z.RSD.CN))

knitr::kable(cor.df, digits = 2, caption = "Spearman Correlation structure (<z.RSD>) - sequence (1/CN)")
```
