...
all.betas <- list("strong.sel", "medium.sel", "weak.sel", "no.sel")
  ...
  for (beta in all.betas)  {
    ...
    gaps = c(18, 19, 47, 48, 49, 50, 51, 52, 53, 54, 55, 148, 149, 150, 151)
    ...
    exp.Pn.sd = apply(m.exp.Pn, 2, sd, na.rm = T)/sqrt(n.prot) * 1.96
    ...
    no.gaps = colSums(matrix(as.numeric(!is.na(m.exp.smooth.norm.dr.squarei)), nrow = n.prot))
    exp.smooth.norm.MSDi = colMeans(m.exp.smooth.norm.dr.squarei, na.rm = T )
    exp.smooth.norm.MSDi.sd = apply(m.exp.smooth.norm.dr.squarei, 2, sd, na.rm = T)/sqrt(no.gaps) * 1.96
  ...
  pdb <- read.pdb(pdbs.fname, ATOM.only = T)
  sel <- atom.select(pdb)
  b.factor <- pdb$atom$b
  site = as.numeric(pdb$atom[sel$atom, c("resno")])[1:1203]
  MSDi.fit.pdb <- vec2resno(as.vector(MSDi.fit), site)
  exp.MSDi.pdb <- vec2resno(as.vector(exp.smooth.norm.MSDi), site)
