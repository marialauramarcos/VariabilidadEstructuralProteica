...
n.sites = length(ref.pdb$xyz[inds$xyz])/3
AA = ref.pdb.ca$resid
...
```{r correlation-da-WCN} 
## calculate correlations between profiles
r.min.da.CA.WCN = cor(WCN[-active.site.index], m.min.da.CM.CA$min.da.CA[-active.site.index])
r.min.da.CM.WCN = cor(WCN[-active.site.index], m.min.da.CM.CA$min.da.CM[-active.site.index])

cor.df = data.frame("comparison" = c("WCN vs min.da.CA", "WCN vs min.da.CM"), "R" = c(r.min.da.CA.WCN, r.min.da.CM.WCN))

knitr::kable(cor.df, digits = 2, caption = "Correlation between WCN and the minimal distance to the active site")

## calculate correlations between profiles
r.sum.inv.da.CA.WCN = cor(WCN[-active.site.index], m.sum.inv.da.CM.CA$sum.inv.da.CA[-active.site.index])
r.sum.inv.da.CM.WCN = cor(WCN[-active.site.index], m.sum.inv.da.CM.CA$sum.inv.da.CM[-active.site.index])

cor.df = data.frame("comparison" = c("WCN vs sum.inv.da.CA", "WCN vs sum.inv.da.CM"), "R" = c(r.sum.inv.da.CA.WCN, r.sum.inv.da.CM.WCN))

knitr::kable(cor.df, digits = 2, caption = "Correlation between WCN and the sum.inv distance to the active site")
```
...
plot(x = p.accept.mut, y = log(WCN))
plot(x = p.accept.mut, y = score)
plot(x = score, y = log(WCN)) 

## calculate corelations
r.p.accept.mut.WCN = cor(p.accept.mut, log(WCN))
r.p.accept.mut.score = cor(p.accept.mut, score)
r.score.WCN = cor(score, log(WCN))

cor.df = data.frame("comparison" = c("paccept no-selection vs log(WCN)", "paccept no-selection vs score", "score vs log(WCN)"),
                    "R" = c(r.p.accept.mut.WCN , r.p.accept.mut.score, r.score.WCN))
...
plot(x = p.accept.medium.sel, y = log(WCN))
plot(x = p.accept.medium.sel, y = score)
plot(x = score, y = log(WCN))

## calculate corelations
r.p.accept.medium.sel.WCN = cor(p.accept.medium.sel, log(WCN))
r.p.accept.medium.sel.score = cor(p.accept.medium.sel, score)
r.score.WCN = cor(score, log(WCN))

cor.df = data.frame("comparison" = c("paccept selection vs log(WCN)", "paccept selection vs score", "score vs log(WCN)"),
                    "R" = c(r.p.accept.medium.sel.WCN , r.p.accept.medium.sel.score, r.score.WCN))
...
r.exp.mean.z.RSD.WCN = cor(d.exp$mean.z.RSD, log(WCN))
r.mut.mean.z.RSD.WCN = cor(d.mut$mean.z.RSD, log(WCN))
r.medium.mean.z.RSD.WCN = cor(d.medium$mean.z.RSD, log(WCN))

cor.df = data.frame("comparison" = c("<z.RSD> experimental vs log(WCN)", "<z.RSD> no-selection vs log(WCN)", "<z.RSD> selection vs log(WCN)"),
                    "R" = c(r.exp.mean.z.RSD.WCN , r.mut.mean.z.RSD.WCN, r.medium.mean.z.RSD.WCN))

knitr::kable(cor.df, digits = 2, caption = "Pearson Correlation structure (<z.RSD>) - sequence (WCN)")

## calculate correlations
r.exp.mean.z.RSD.WCN = cor(d.exp$mean.z.RSD, log(WCN), method = "spearman")
r.mut.mean.z.RSD.WCN = cor(d.mut$mean.z.RSD, log(WCN), method = "spearman")
r.medium.mean.z.RSD.WCN = cor(d.medium$mean.z.RSD, log(WCN), method = "spearman")
