...
d = ddply(dat,c("dataset", "site", "AA", "shell"), function(x) {
                ...
                ndata = sum(!is.na(x$r.SDF))
                se.r.SDF = sd(x$r.SDF, na.rm = T) / sqrt(ndata)
                data.frame(mean.r.SDF, ndata, se.r.SDF)
})
...
plot(x = mean.r.square.dif.MSF.exp, 
     ...
     xlab = "exp.mean.r.square.dif.MSF", 
     ylab = "no.selection.mean.r.square.dif.MSF")
abline(0, 1)

plot(x = mean.r.square.dif.MSF.exp, 
     y = mean.r.square.dif.MSF.medium, 
     xlab = "exp.mean.r.square.dif.MSF", 
     ylab = "selection.mean.r.square.dif.MSF")
abline(0, 1)

plot(x = mean.r.square.dif.MSF.mut, 
     y = mean.r.square.dif.MSF.medium, 
     xlab = "no.selection.mean.r.square.dif.MSF", 
     ylab = "selection.mean.r.square.dif.MSF")
abline(0, 1)
```
...
plot(x = mean.r.square.dif.MSF.exp, 
     y = mean.r.dri2.exp, 
     xlab = "exp.mean.r.square.dif.MSF", 
     ylab = "exp.mean.r.dri2")
abline(0, 1)

plot(x = mean.r.square.dif.MSF.medium, 
     y = mean.r.dri2.medium, 
     xlab = "selection.mean.r.square.dif.MSF", 
     ylab = "selection.mean.r.dri2")
abline(0, 1)

plot(x = mean.r.square.dif.MSF.mut, 
     y = mean.r.dri2.mut, 
     xlab = "no.selection.mean.r.square.dif.MSF", 
     ylab = "no.selection.mean.r.dri2")
abline(0, 1)
