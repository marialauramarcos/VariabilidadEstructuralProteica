...
dri2.exp.fname = file.path(data.dir, paste(family, "_R0_", R0, "_beta_strong.sel_K.analysis_Keff_out_m.exp.norm.dr.squarei.csv", sep = ""))
Pn.exp.fname = file.path(data.dir, paste(family,"_R0_", R0, "_beta_strong.sel_K.analysis_Keff_out_m.exp.Pn.csv", sep = ""))
energy.exp.fname = file.path(data.dir, paste(family, "_R0_", R0, "_beta_strong.sel_K.analysis_Keff_out_m.exp.va.csv", sep = ""))
...
We use the Elastic Network Model with R0 = `r R0` for the `r family` family. 
We use as reference for the ENM the protein with pdb code `r reference`.
The `r family` family is of the type of `r type` and consists of the following proteins:
...
# note: rows of dri2 are proteins, columns are sites
...
## read information about minimal active site distances
m.min.da.CM = read.csv(file.path(data.dir, paste(p.ref, "_min.da.CM.ca.csv", sep = "")))

z.RSD.exp = d$mean.z.RSD[d$dataset == "exp"]
z.RSD.strong = d$mean.z.RSD[d$dataset == "mut+sel"]
z.RSD.mut = d$mean.z.RSD[d$dataset == "mut"]

plot(y = z.RSD.exp - z.RSD.strong, x = m.min.da.CM$min.da.ca, xlab = "minimal distance to active site (between CAs)")
plot(y = z.RSD.exp - z.RSD.mut, x = m.min.da.CM$min.da.CM, xlab = "minimal distance to active site (between CMs)")

print("agregar correlación")
...
## experimental
exp.mean.z.RSD.vec <- vec2resno(as.vector(dat.exp$mean.z.RSD), sort(rep(1:ncol(pdb.xyz.ca),3)))
write.pdb(xyz = c(pdb.xyz.ca), b = exp.mean.z.RSD.vec, file =  paste(p.ref, "_exp.pdb", sep = ""))

## mut
mut.mean.z.RSD.vec <- vec2resno(as.vector(dat.mut$mean.z.RSD), sort(rep(1:ncol(pdb.xyz.ca),3)))
write.pdb(xyz = c(pdb.xyz.ca), b = mut.mean.z.RSD.vec, file = paste(p.ref, "_mut.pdb", sep = ""))

## mut + sel / strong
strong.mean.z.RSD.vec <- vec2resno(as.vector(dat.strong$mean.z.RSD), sort(rep(1:ncol(pdb.xyz.ca),3)))
write.pdb(xyz = c(pdb.xyz.ca), b = strong.mean.z.RSD.vec, file = paste(p.ref, "_strong.pdb", sep = ""))
```
...
# print("Warning: modeComparison <- na.omit(modeComparison)")
