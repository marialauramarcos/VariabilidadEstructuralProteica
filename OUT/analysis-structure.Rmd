...
site.info = read.csv(site.info.fname, header = T, sep = ",")[1:151, ]
...
plot(y = z.RSD.exp-z.RSD.strong, x = m.min.da.CM$min.da.ca[1:151])
plot(y = (z.RSD.exp-z.RSD.mut), x = m.min.da.CM$min.da.CM[1:151])
...
# Write pdb files for MSDi.
  ...
  pdb = ReadCA(reference.pdb.fname, chain.p.ref)$xyz

  mean.exp.z.RSD.pdb <- vec2resno(as.vector(dat.exp$mean.z.RSD), sort(rep(1:ncol(pdb),3)))
  write.pdb(xyz = c(pdb), b = mean.exp.z.RSD.pdb, file = paste(p.ref, "_exp.pdb", sep = ""))

  mean.mut.z.RSD.pdb <- vec2resno(as.vector(dat.mut$mean.z.RSD), sort(rep(1:ncol(pdb),3)))
  write.pdb(xyz = c(pdb), b = mean.mut.z.RSD.pdb, file = paste(p.ref, "_mut.pdb", sep = ""))
  
  mean.strong.z.RSD.pdb <- vec2resno(as.vector(dat.strong$mean.z.RSD), sort(rep(1:ncol(pdb),3)))
  write.pdb(xyz = c(pdb), b = mean.strong.z.RSD.pdb, file = paste(p.ref, "_strong.pdb", sep = ""))
```
