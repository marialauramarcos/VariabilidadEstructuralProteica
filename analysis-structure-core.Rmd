...
For the core we considered sites with no gaps in the whole multiple alignment.
...
sim.data.medium.fname = file.path(data.dir, paste(family, "_R0_", R0, "_beta_medium.sel_out_df.data.csv", sep = ""))
```
...
```{r read-the-core, comment = ""}
# indexes.core
indexes.core = as.numeric(read.csv(file.path("DATA", paste("indexes.core", "_", family, ".csv", sep = "")))[1, -1])
```
...
## medium
dri2.medium = read.csv(dri2.medium.fname, header = T)[, indexes.core]
...
p = ggplot(d, aes(x = site, y = mean.z.RSD, col = dataset, fill = dataset)) +
    geom_point() +
    geom_errorbar(aes(ymin = mean.z.RSD - f95 * se.z.RSD, ymax = mean.z.RSD + se.z.RSD), alpha = .4, show.legend = F) +
    geom_vline(xintercept = active.sites, show.legend = F) +
    geom_vline(xintercept = contacts.active.sites, col = "grey", show.legend = F) +
    labs(x = "site", y = "structural divergence(<z.RSD>)") +
...
```{r comparison-mean-z.RSD-vs-site, fig.cap = "Black lines are the active sites and grey lines are their contacts"}
...
active.sites = active.sites[!duplicated(active.sites)]
contacts.active.sites = contacts.active.sites[!duplicated(active.sites)]

# new.indexes
new.active.index = c()
for (i in (1:(length(active.sites)))){
  new.active.index[i] = which(indexes.core == active.sites[i])
}

new.contacts.active.index = c()
for (i in (1:(length(contacts.active.sites)))){
  new.contacts.active.index[i] = which(indexes.core == contacts.active.sites[i])
}

active.sites = new.active.index
contacts.active.sites = new.contacts.active.index
...
plot(y = (d.exp$mean.z.RSD - d.mut$mean.z.RSD)[-active.sites], x = m.min.da.CM.CA$min.da.CA[indexes.core][-active.sites], xlab = "minimal distance to active site (between CAs)", ylab = "<z.RSD> experimental - <z.RSD> no-selection")
plot(y = (d.exp$mean.z.RSD - d.mut$mean.z.RSD)[-active.sites], x = m.min.da.CM.CA$min.da.CM[indexes.core][-active.sites], xlab = "minimal distance to active site (between CMs)", ylab = "<z.RSD> experimental - <z.RSD> no-selection")

## calculate correlations between profiles
r.min.da.CA = cor((d.exp$mean.z.RSD - d.mut$mean.z.RSD)[-active.sites], m.min.da.CM.CA$min.da.CA[indexes.core][-active.sites])
r.min.da.CM = cor((d.exp$mean.z.RSD - d.mut$mean.z.RSD)[-active.sites], m.min.da.CM.CA$min.da.CM[indexes.core][-active.sites])

cor.df = data.frame("comparison" = c("(<z.RSD> experimental - <z.RSD> no-selection) vs min.da.CA", "(<z.RSD> experimental - <z.RSD> no-selection) vs min.da.CM"), "R" = c(r.min.da.CA, r.min.da.CM))
...
# comparison of means.z.scores with the minimum distance to the functional sites
plot(y = (d.exp$mean.z.RSD - d.medium$mean.z.RSD)[-active.sites], x = m.min.da.CM.CA$min.da.CA[indexes.core][-active.sites], xlab = "minimal distance to active site (between CAs)", ylab = "<z.RSD> experimental - <z.RSD> selection")
plot(y = (d.exp$mean.z.RSD - d.medium$mean.z.RSD)[-active.sites], x = m.min.da.CM.CA$min.da.CM[indexes.core][-active.sites], xlab = "minimal distance to active site (between CMs)", ylab = "<z.RSD> experimental - <z.RSD> selection")

## calculate correlations between profiles
r.min.da.CA = cor((d.exp$mean.z.RSD - d.medium$mean.z.RSD)[-active.sites], m.min.da.CM.CA$min.da.CA[indexes.core][-active.sites])
r.min.da.CM = cor((d.exp$mean.z.RSD - d.medium$mean.z.RSD)[-active.sites], m.min.da.CM.CA$min.da.CM[indexes.core][-active.sites])

cor.df = data.frame("comparison" = c("(<z.RSD> experimental - <z.RSD> selection) vs min.da.CA", "(<z.RSD> experimental - <z.RSD> selection) vs min.da.CM"), "R" = c(r.min.da.CA, r.min.da.CM))
...
plot(y = (d.exp$mean.z.RSD - d.mut$mean.z.RSD)[-active.sites], x = m.sum.inv.da.CM.CA$sum.inv.da.CA[indexes.core][-active.sites], xlab = "sum.inv distance to active site (between CAs)", ylab = "<z.RSD> experimental - <z.RSD> no-selection")
plot(y = (d.exp$mean.z.RSD - d.mut$mean.z.RSD)[-active.sites], x = m.sum.inv.da.CM.CA$sum.inv.da.CM[indexes.core][-active.sites], xlab = "sum.inv distance to active site (between CMs)", ylab = "<z.RSD> experimental - <z.RSD> no-selection")

## calculate correlations between profiles
r.sum.inv.da.CA = cor((d.exp$mean.z.RSD - d.mut$mean.z.RSD)[-active.sites], m.sum.inv.da.CM.CA$sum.inv.da.CA[indexes.core][-active.sites])
r.sum.inv.da.CM = cor((d.exp$mean.z.RSD - d.mut$mean.z.RSD)[-active.sites], m.sum.inv.da.CM.CA$sum.inv.da.CM[indexes.core][-active.sites])

cor.df = data.frame("comparison" = c("(<z.RSD> experimental - <z.RSD> no-selection) vs sum.inv.da.CA", "(<z.RSD> experimental - <z.RSD> no-selection) vs sum.inv.da.CM"), "R" = c(r.sum.inv.da.CA, r.sum.inv.da.CM))
...
# comparison of means.z.scores with the sum.inv distance to the functional sites
plot(y = (d.exp$mean.z.RSD - d.medium$mean.z.RSD)[-active.sites], x = m.sum.inv.da.CM.CA$sum.inv.da.CA[indexes.core][-active.sites], xlab = "sum.inv distance to active site (between CAs)", ylab = "<z.RSD> experimental - <z.RSD> selection")
plot(y = (d.exp$mean.z.RSD - d.medium$mean.z.RSD)[-active.sites], x = m.sum.inv.da.CM.CA$sum.inv.da.CM[indexes.core][-active.sites], xlab = "sum.inv distance to active site (between CMs)", ylab = "<z.RSD> experimental - <z.RSD> selection")

## calculate correlations between profiles
r.sum.inv.da.CA = cor((d.exp$mean.z.RSD - d.medium$mean.z.RSD)[-active.sites], m.sum.inv.da.CM.CA$sum.inv.da.CA[indexes.core][-active.sites])
r.sum.inv.da.CM = cor((d.exp$mean.z.RSD - d.medium$mean.z.RSD)[-active.sites], m.sum.inv.da.CM.CA$sum.inv.da.CM[indexes.core][-active.sites])

cor.df = data.frame("comparison" = c("(<z.RSD> experimental - <z.RSD> selection) vs sum.inv.da.CA", "(<z.RSD> experimental - <z.RSD> selection) vs sum.inv.da.CM"), "R" = c(r.sum.inv.da.CA, r.sum.inv.da.CM))
...
## calculate correlations between profiles
r.min.da.CA.CN = cor(CN[-active.sites], m.min.da.CM.CA$min.da.CA[indexes.core][-active.sites])
r.min.da.CM.CN = cor(CN[-active.sites], m.min.da.CM.CA$min.da.CM[indexes.core][-active.sites])
...
## calculate correlations between profiles
r.sum.inv.da.CA.CN = cor(CN[-active.sites], m.sum.inv.da.CM.CA$sum.inv.da.CA[indexes.core][-active.sites])
r.sum.inv.da.CM.CN = cor(CN[-active.sites], m.sum.inv.da.CM.CA$sum.inv.da.CM[indexes.core][-active.sites])
...
score = consurf$SCORE[indexes.core]
...
p.accept.mut = p.accept.mut[indexes.core]
p.accept.medium.sel = p.accept.medium.sel[indexes.core]

# compare exp vs no sel profiles
## plot
par(pty = "s")

plot(x = p.accept.mut, y = CN, xlab = "log(paccept no-selection)")
plot(x = p.accept.mut, y = score, xlab = "log(paccept no-selection)")
plot(x = score, y = CN)

## calculate corelations
r.p.accept.mut.CN = cor(log(p.accept.mut), CN)
r.p.accept.mut.score = cor(log(p.accept.mut), score)
r.score.CN = cor(score, CN)

cor.df = data.frame("comparison" = c("log(paccept no-selection) vs CN", "log(paccept no-selection) vs score", "score vs CN"),
...
plot(x = log(p.accept.medium.sel), y = CN, xlab = "log(paccept selection)")
plot(x = log(p.accept.medium.sel), y = score, xlab = "log(paccept selection)")
plot(x = score, y = CN)
...
cor.df = data.frame("comparison" = c("log(paccept selection) vs CN", "log(paccept selection) vs score", "score vs CN"),
