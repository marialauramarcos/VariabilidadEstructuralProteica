...
p = ggplot(d, aes(x = site, y = mean.z.RSD, col = dataset, fill = dataset)) +
    geom_point() +
    geom_errorbar(aes(ymin = mean.z.RSD - f95 * se.z.RSD, ymax = mean.z.RSD +  f95 * se.z.RSD), alpha = .4, show.legend = F) +
...
```{r comparisons-between-profiles, fig.cap = "Yellow points are the active sites and green points are their contacts"}
